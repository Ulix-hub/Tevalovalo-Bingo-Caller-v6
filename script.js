...
let pool = []; let called = []; let autoTimer = null; let isLocked = false; let pinCode = "";
...
// -------- App logic ----------
function applyTheme(cls){ const body=document.body; THEME_CLASSES.forEach(c=>body.classList.remove(c)); body.classList.add(cls); }
function pickRandomTheme(){ return THEME_CLASSES[Math.floor(Math.random()*THEME_CLASSES.length)]; }
function renderGrid(){ grid.innerHTML=''; for(let n=1;n<=90;n++){ const b=document.createElement('div'); b.className='cell'+(called.includes(n)?' called':''); b.textContent=n; grid.appendChild(b); } }
function speakNumber(n){ try{ if(!speakToggle.checked) return; const msg=new SpeechSynthesisUtterance(String(n)); msg.rate=.95; msg.pitch=1; speechSynthesis.cancel(); speechSynthesis.speak(msg);}catch{} }
...
function startGame(){
  ...
  applyTheme(chosen);
  pool = Array.from({length:90}, (_,i)=>i+1); called = []; shuffle(pool);
  renderGrid(); updateUI(); nextBtn.disabled = false; resetBtn.disabled = false;
}
function nextCall(){ if(pool.length===0 || isLocked) return; const n=pool.pop(); called.push(n); renderGrid(); playSound(); speakNumber(n); updateUI(); }
function undoLast(){ if(!called.length || isLocked) return; const n=called.pop(); pool.push(n); renderGrid(); updateUI(); }
function resetAll(){ if(isLocked) return; stopAuto(); pool=[]; called=[]; renderGrid(); updateUI(); currentEl.textContent='—'; lastFiveEl.textContent='Last 5: —'; }
function shuffle(arr){ for(let i=arr.length-1;i>0;i--){ const j=Math.floor(Math.random()*(i+1)); [arr[i],arr[j]]=[arr[j],arr[i]]; } }
function toggleAuto(){ if(autoTimer){ stopAuto(); return; } if(isLocked) return; const s=Math.max(2, Math.min(60, parseInt(autoSeconds.value||'6',10))); autoSeconds.value=s; autoTimer=setInterval(()=>{ if(pool.length===0||isLocked){ stopAuto(); return; } nextCall(); }, s*1000); updateUI(); }
function stopAuto(){ if(autoTimer){ clearInterval(autoTimer); autoTimer=null; updateUI(); } }
function lockBoard(){ let p=prompt('Set a 4-digit PIN to unlock (optional). Leave blank for no PIN:'); if(p===null) return; p=(p||'').trim(); if(p && !/^\d{4}$/.test(p)){ alert('PIN must be 4 digits.'); return; } pinCode=p; isLocked=true; stopAuto(); lockOverlay.hidden=false; updateUI(); }
function unlockBoard(){ if(!isLocked) return; if(pinCode){ const entry=prompt('Enter PIN:'); if(entry!==pinCode){ alert('Incorrect PIN.'); return; } } isLocked=false; lockOverlay.hidden=true; updateUI(); }
...
function downloadBoardPNG(){
  ...
  for(let r=0;r<rows;r++){ for(let c=0;c<cols;c++){ const n=r*10+c+1; const x=gx+c*cellW, y=gy+r*cellH; const isCalled=called.includes(n);
  ...
  const url=exportCanvas.toDataURL('image/png'); const a=document.createElement('a'); a.href=url; a.download='bingo-board.png'; a.click(); setTimeout(()=>URL.revokeObjectURL?.(url),500);
}
