...
// script.js — robust DOM-ready version + unlock fixes
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function downloadBoardPNG(){
    ...
    setTimeout(() => { if (URL.revokeObjectURL) URL.revokeObjectURL(url); }, 500);
  }
  ...
  // Emergency keyboard unlock: press U
  window.addEventListener('keydown', e => { if(e.key === 'u' || e.key === 'U'){ unlockBoard(); } });

  // init
  renderGrid();
  updateUI();

  console.log('[Bingo Caller] script.js loaded, DOM ready');
});
